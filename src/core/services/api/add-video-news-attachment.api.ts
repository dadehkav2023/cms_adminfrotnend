...
const url = MainUrl + "/api/News/VideoNews/Attachment/NewAttachment";

const AddVideoNewsAttachmentApi = async (
  value: any
): Promise<AxiosResponse<IAxiosResult>> => {
  const formOfValues = new FormData();
  formOfValues.set("Title", value.Title);
  formOfValues.set("VideoNewsId", value.Id);
  formOfValues.set("VideoPath", value.File);
  return await methods.post(url, formOfValues, {
    headers: { "Content-Type": "multipart/form-data" },
  });
};

export const UseAddVideoNewsAttachment = () => {
  // const history = useHistory();
  return useMutation((obj: any) => AddVideoNewsAttachmentApi(obj), {
    onSuccess: (value) => {
      value.data
