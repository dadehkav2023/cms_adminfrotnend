...
const url = MainUrl + "/api/News/PhotoNews/Attachment/EditAttachment";

const EditPhotoNewsAttachmentApi = async (
  value: any
): Promise<AxiosResponse<IAxiosResult>> => {
  const formOfValues = new FormData();
  formOfValues.set("Id", value.Id);
  formOfValues.set("Title", value.Title);
  formOfValues.set("ImagePath", value.File ? value.File[0] : null);
...
export const UseEditPhotoNewsAttachment = () => {
  const history = useHistory();
  return useMutation((obj: any) => EditPhotoNewsAttachmentApi(obj), {
    onSuccess: (value) => {
      value.data
