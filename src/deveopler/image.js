...
export default {
    ...
    register: function (info, response) {
        ...
        for (let i = 0, len = fileList.length, file; i < len; i++) {
            ...
            if (info.isUpdate) {
                this.plugins.image.update_src.call(this,
                    
                   
                    "https://api.admin.cms.sabakorg.ir/" +
                    fileList[i].url.replace(/\\/g, "/")
                    ,
                    
                    info.element, file);
                break;
            } else {
                this.plugins.image.create_image.call(this,
                    "https://api.admin.cms.sabakorg.ir/" +
                    fileList[i].url.replace(/\\/g, "/")
                    , info.anchor, info.inputWidth, info.inputHeight, info.align, file);
            }
        }
