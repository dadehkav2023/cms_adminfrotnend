...
import Styles from "./QuickAccessList.module.scss";
...
export interface QuickAccessListProps {}

const QuickAccessList: React.FC<QuickAccessListProps> = () => {
  ...
  const { data, isError, isLoading, isSuccess, mutate } = UseGetQuickAccess();
        ...
        <Form>
          ...
          >
            <Container className={Styles.container} fluid>
              ...
              {!data?.data.result.quickAccessList[0] ? (
                <h1 className={Styles.noneFound}>موردی یافت نشد</h1>
              ) : (
                data?.data.result.quickAccessList.map(
                  (item: any, index: any) => {
                    return (
                      <Row key={index}>
                        <Col xs={2}>
                        ...
                        <Col xs={3}>
                          <div className={Styles.buttonDiv}>
                            <Link
                              to={
                                "/QuickAccessManagement/AlterQuickAccess/" +
                                item.id
                              }
                            >
                              <SimpleSubmitButton
